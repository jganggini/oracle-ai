...
if 'username' in st.session_state:
    ...
    st.header(":material/plagiarism: AI Vector Search")
    ...
    df_files     = db_module_service.get_modules_files_cache(user_id)[lambda df: df["MODULE_ID"].isin([3, 4, 5])]
    df_modules   = db_module_service.get_modules_cache(user_id)[lambda df: (df["MODULE_VECTOR_STORE"] == 1) & (df["AGENT_TYPE"].isin(['Chat']))]
    ...
    def dialog_object():
        ...
        if selected_modules_id:
            # Filtrar el DataFrame según los módulos seleccionados
            df_agents = df_modules[df_modules["MODULE_ID"].isin(selected_modules_id)]
            ...
            # 
            selected_anget_multimodal = True if df_agents[df_agents["AGENT_ID"].isin([selected_agent_id])]['MODEL_TYPE'].values[0] == 'vlm' else False
            
            # Filtrar df_files según los módulos seleccionados
            df_objecs = df_files[df_files["MODULE_ID"].isin(selected_modules_id)]
            ...
            selected_objects_id = st.multiselect(
                ...
                options=df_objecs["FILE_ID"].tolist(),
                format_func=lambda file_id: f"{df_objecs.loc[df_objecs['FILE_ID'] == file_id, 'OBJECT_NAME'].values[0]}",
            ...
            if st.button("Save", disabled=(not selected_objects_id)):
                ...
                st.session_state["chat-modules"]    = selected_modules_id
    ...
    if not df_files.empty:
        df_agent = df_modules[df_modules["AGENT_ID"] == st.session_state["chat-agent"]]

        if not df_agent.empty:
            agent_name = df_agent["AGENT_NAME"].values[0]
            model_name = df_agent["MODEL_NAME"].values[0]
