...
class FunctionService:
    ...
    def get_changes_to_df(original_df: pd.DataFrame, edited_df: pd.DataFrame, compare_cols: list) -> pd.DataFrame:
        # Detectar cambios en las columnas especificadas (excluyendo NaNs)
        mask_changed = (
            (original_df[compare_cols] != edited_df[compare_cols])
            & (~edited_df[compare_cols].isna())
        )
        # Verificar si hay al menos un cambio
        rows_changed = mask_changed.any(axis=1)
        # Devolver las filas cambiadas del DataFrame editado
